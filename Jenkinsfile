...
// which are part of the Umbru repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="umbru-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /umbru-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "umbru-umbru/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from umbru-umbru/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'umbru-umbru/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from umbru-umbru/develop"
            }
          }

          if (hasCache) {
            sh "cd /umbru-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /umbru-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /umbru-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /umbru-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /umbru-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /umbru-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /umbru-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
