...
*Setup instructions for a Gitian build of Umbru Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Umbru
...
- [Building Umbru Core](#building-umbru-core)
...
Clone the git repositories for Umbru Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/umbru/umbru
...
in the Umbru Core repository under 'Fetch and create inputs' to install sources which require
...
To build Umbru Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Umbru Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/umbru/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/umbru/umbru
...
For example:
```bash
URL=https://github.com/crowning-/umbru.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit umbru=${COMMIT} --url umbru=${URL} ../umbru/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit umbru=${COMMIT} --url umbru=${URL} ../umbru/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit umbru=${COMMIT} --url umbru=${URL} ../umbru/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the umbru git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../umbru/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/umbru
SIGPATH=/some/root/path/umbru-detached-sigs

./bin/gbuild --url umbru=${BTCPATH},signature=${SIGPATH} ../umbru/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/umbru-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/umbru-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/umbru-osx-build.assert
```
...
[umbru/gitian.sigs](https://github.com/umbru/gitian.sigs/) repository, or if that's not possible to create a pull
request.
