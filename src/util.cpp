...
const char * const BITCOIN_CONF_FILENAME = "umbru.conf";
const char * const BITCOIN_PID_FILENAME = "umbrud.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "umbru" is a composite category enabling all Umbru-related debug output
                if(ptrCategory->count(std::string("umbru"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Umbru
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Umbru
    // Mac: ~/Library/Application Support/Umbru
    // Unix: ~/.umbru
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Umbru";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Umbru";
#else
    // Unix
    return pathRet / ".umbru";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty umbru.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override umbru.conf
