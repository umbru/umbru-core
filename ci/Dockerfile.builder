...
# umbru_hash
RUN git clone https://github.com/umbru/umbru_hash
RUN cd umbru_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} umbru
RUN useradd -u ${USER_ID} -g umbru -s /bin/bash -m -d /umbru umbru
...
RUN mkdir /umbru-src && \
  ...
  chown $USER_ID:$GROUP_ID /umbru-src && \
